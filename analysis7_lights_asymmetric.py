...
def main():
    ...
    print(f"Lights asymmetric sweep: {len(rates_h)} x {len(rates_v)} grid")
    for i, rh in enumerate(rates_h):
        for j, rv in enumerate(rates_v):
            th, tv, T = run_lights(rate_h=rh, rate_v=rv, T=200.0, period=20.0, seed=71)
            all_t = th + tv
    ...
    ax.set_title("Traffic light: mean passing time vs asymmetric spawn rates")
